...
public class OrderTest {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("list of order with products belong to category “Baby are:=- ");
        orders.stream()
                //.map(o -> o.getProducts())
                .filter(order -> order.getProducts().stream()
                        .anyMatch(product -> product.getCategory()=="Baby"))
                .collect(Collectors.toList())
                .forEach(System.out::println);
        ...
        products.stream()
               ...
               // .map(p -> p.setPrice(p.getPrice() * 0.90))
                .forEach(System.out::println);
        ...
        orders.stream()
                .filter(order -> order.getCustomer().getTier()==2 && order.getOrderDate().isAfter(LocalDate.of(2021,2,01))
                        && order.getOrderDate().isBefore(LocalDate.of(2021,4,01)))
        ...
        double sum = orders.stream()
                .filter(order -> order.getOrderDate().getMonthValue() == 02 && order.getOrderDate().getYear() == 2021)
               // .peek(System.out::println)
                .flatMap(order -> order.getProducts().stream())
                //.collect(Collectors.toList())
                .mapToDouble(Product::getPrice)
                .sum();
        System.out.println("total lump sum spend on Feb2021:"+sum);

        // Exercise 9 — Calculate order average payment placed on 14-Mar-2021
        System.out.println("\n\nExercise 9 — Calculate order average payment placed on 14-Mar-2021");
        DateTimeFormatter df = DateTimeFormatter.ofPattern("dd-MMM-yyyy");

        System.out.println("Avg amt :"+orders.stream()
        ...
        DoubleSummaryStatistics stats = products.stream()
                ...
                .summaryStatistics();
        System.out.printf(
                " Count: %S,Average: %S, Max: %S,Min: %S,Sum: %S",
        ...
        orders.stream()
                ...
                .forEach((customers,order)->
                        System.out.println(customers.getName()+":"+order));
